...
class Graph:
    def __init__(self, head_size, image_size, l2_reg_lambda=3.0,
        ...
        self.input_image__ = tf.compat.v1.placeholder(tf.float64, shape=[None, image_size, embedding_len], name='input_image')
        ...
        self.input_image_ = tf.reshape(self.input_image__, [-1, embedding_len])
        ...
        with tf.name_scope('calculate_cos_simi'):
            ...
            combine_image = image_vector
            combine_text = headline_vector
        ...
        with tf.compat.v1.name_scope("prediction"):
            vector = tf.concat([headline_vector, image_vector], 1)
            W = tf.Variable(tf.compat.v1.truncated_normal([self.final_len * 2, 2], stddev=0.1, dtype=tf.float64),
                            dtype=tf.float64)

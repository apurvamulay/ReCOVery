...
tf.compat.v1.flags.DEFINE_integer("image_size", 14, "the length of body matrix")
...
tf.compat.v1.flags.DEFINE_integer("num_epochs", 1000, "Number of training epochs (Default: 100)")
tf.compat.v1.flags.DEFINE_integer("num_batchs", 32, "Number of batchs per fold (Default: 100)")
...
def train(x_train_head, x_train_image, y_train, modelfolder, boarddir):
                ...
                for i in range(5):  # 5-fold
                    ...
                    for batch in range(FLAGS.num_batchs):
                        ...
                        _, train_accuracy, trainloss = \
                            sess.run([layer.train_op, layer.accuracy, layer.loss], feed_dict={layer.input_headline__: x_batch_head,
                                                                                              layer.input_image__: x_batch_image,
                    ...
                    dev_accuracy_fake, dev_loss = sess.run(
                        [layer.accuracy, layer.loss], feed_dict={layer.input_headline__: x_dev_head,
                                                                               layer.input_image__: x_dev_image,
...
def main(_):
    ...
    with open('～/safe/dic_embedding.json', 'r') as f:
        dic = json.load(f)
    ...
    for key, value in dic.items():
        ...
        tmp = [float(i) for j in value['image'] for i in j]
        tmp = np.array(tmp).reshape((-1, 300))
        x_image.append(tmp)
    ...
    x_image_train, x_image_test, y_train, y_test = train_test_split(x_image, y, test_size=0.2, random_state=4)

    modelfolder = '～/safe/ckp/'
    log = '～/safe/runs/logs/'

    train(x_head_train, x_image_train, y_train, modelfolder, log)
